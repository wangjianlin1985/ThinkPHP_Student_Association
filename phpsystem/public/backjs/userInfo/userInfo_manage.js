				...
				if ($("#userInfoEditForm").form("validate")) {
					...
					if(!$("#userInfoEditForm").form("validate")) {
						...
						$("#userInfoEditForm").form({
						    url: backURL + getVisitPath("UserInfo") + "/update",
						    onSubmit: function(){
								if($("#userInfoEditForm").form("validate"))  {
				                	$.messager.progress({
										text : "正在提交数据中...",
	...
	userInfo_manage_tool = {
		...
		exportExcel: function() {
			$("#userInfoQueryForm").form({
			    url: backURL + getVisitPath("UserInfo") + "/outToExcel",
			});
			//提交表单
			...
			if (rows.length > 0) {
				$.messager.confirm("确定操作", "您正在要删除所选的记录吗？", function (flag) {
					if (flag) {
						...
						$.ajax({
							type : "POST",
							url :  backURL + getVisitPath("UserInfo") + "/deletes",
							data : {
				...
				$.ajax({
					...
					success : function (userInfo, response, status) {
						...
						if (userInfo) { 
							...
							$("#userInfo_userPhotoImg").attr("src", publicURL + userInfo.userPhoto);
