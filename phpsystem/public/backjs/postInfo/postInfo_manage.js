				...
				if ($("#postInfoEditForm").form("validate")) {
					...
					if(!$("#postInfoEditForm").form("validate")) {
						...
						$("#postInfoEditForm").form({
						    url: backURL + getVisitPath("PostInfo") + "/update",
						    onSubmit: function(){
								if($("#postInfoEditForm").form("validate"))  {
				                	$.messager.progress({
										text : "正在提交数据中...",
	...
	postInfo_manage_tool = {
		...
		exportExcel: function() {
			$("#postInfoQueryForm").form({
			    url: backURL + getVisitPath("PostInfo") + "/outToExcel",
			});
			//提交表单
			...
			if (rows.length > 0) {
				$.messager.confirm("确定操作", "您正在要删除所选的记录吗？", function (flag) {
					if (flag) {
						...
						$.ajax({
							type : "POST",
							url :  backURL + getVisitPath("PostInfo") + "/deletes",
							data : {
						...
						if (postInfo) { 
							...
							$("#postInfo_userObj_user_name_edit").combobox({
						        ...
						        onLoadSuccess: function () { //数据加载完毕事件
									$("#postInfo_userObj_user_name_edit").combobox("select", postInfo.userObj);
